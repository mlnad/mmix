...
impl Memory {
    ...
    pub fn read_u32(&self, addr: u64) -> u32 {
        let mut acc = 0u32;
        for i in 0..4u64 {
            acc = (acc << 8) | self.read_byte(addr.wrapping_add(i)) as u32;
        }
        acc
    }
    ...
    pub fn read_u64(&self, addr: u64) -> u64 {
        let mut acc = 0u64;
        for i in 0..8u64 {
            acc = (acc << 8) | self.read_byte(addr.wrapping_add(i)) as u64;
        }
        acc
    }
    ...
    pub fn write_u32(&mut self, addr: u64, val: u32) {
        for i in 0..4u64 {
            self.write_byte(addr.wrapping_add(i), (val >> (24 - i * 8)) as u8);
        }
    }
    ...
    pub fn write_u64(&mut self, addr: u64, val: u64) {
        for i in 0..8u64 {
            self.write_byte(addr.wrapping_add(i), (val >> (56 - i * 8)) as u8);
        }
    }
}
...
mod tests {
    ...
    fn read_uninitialized_returns_zero() {
        ...
        assert_eq!(m.read_byte(u64::MAX), 0);
    ...
    fn byte_roundtrip() {
        ...
        m.write_byte(100, 0xAB);
        assert_eq!(m.read_byte(100), 0xAB);
        assert_eq!(m.read_byte(101), 0);
    }
    ...
    fn u32_big_endian() {
        ...
        assert_eq!(m.read_byte(0), 0x12);
        assert_eq!(m.read_byte(1), 0x34);
        assert_eq!(m.read_byte(2), 0x56);
        assert_eq!(m.read_byte(3), 0x78);
