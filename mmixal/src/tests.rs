...
fn assemble_single_add_reg() {
    ...
    assert_eq!(r.segments[0].bytes.len(), 4);
    let word = u32::from_be_bytes(r.segments[0].bytes[..4].try_into().unwrap());
...
End     TRAP 0,0,0";
    ...
    let word0 = u32::from_be_bytes(r.segments[0].bytes[0..4].try_into().unwrap());
...
Loop    SETL $1,1\n\
    ...
    let word1 = u32::from_be_bytes(r.segments[0].bytes[4..8].try_into().unwrap());
...
Start:  SETL $1,42\n\
        TRAP 0,0,0";
    let r = assemble(src).unwrap();
    assert_eq!(r.segments[0].bytes.len(), 8);
    let word0 = u32::from_be_bytes(r.segments[0].bytes[0..4].try_into().unwrap());
...
fn assemble_byte_values() {
    ...
    assert_eq!(r.segments[0].bytes, vec![1, 2, 3]);
}

#[test]
fn assemble_byte_string() {
    let r = assemble("        BYTE \"AB\"").unwrap();
    assert_eq!(r.segments[0].bytes, vec![b'A', b'B']);
}

#[test]
fn assemble_byte_string_with_escape() {
    let r = assemble("        BYTE \"\\n\\t\\0\"").unwrap();
    assert_eq!(r.segments[0].bytes, vec![b'\n', b'\t', 0]);
}
...
fn assemble_byte_string_with_null_terminator() {
    ...
    assert_eq!(r.segments[0].bytes, vec![b'H', b'i', 0]);
}

#[test]
fn assemble_wyde_values() {
    let r = assemble("        WYDE 0x1234,0x5678").unwrap();
    assert_eq!(r.segments[0].bytes, vec![0x12, 0x34, 0x56, 0x78]);
}

#[test]
fn assemble_tetra_value() {
    let r = assemble("        TETRA 0xDEADBEEF").unwrap();
    assert_eq!(r.segments[0].bytes, vec![0xDE, 0xAD, 0xBE, 0xEF]);
}

#[test]
fn assemble_octa_value() {
    let r = assemble("        OCTA 0x0102030405060708").unwrap();
    assert_eq!(r.segments[0].bytes, vec![0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]);
}
...
fn assemble_and_or_xor() {
    ...
    let w0 = u32::from_be_bytes(r.segments[0].bytes[0..4].try_into().unwrap());
    let w1 = u32::from_be_bytes(r.segments[0].bytes[4..8].try_into().unwrap());
    let w2 = u32::from_be_bytes(r.segments[0].bytes[8..12].try_into().unwrap());
...
String  BYTE    \"Hello, World!\\n\\0\"";
    ...
    assert_eq!(r.segments[0].bytes.len(), 40 + 15);
    ...
    let w9 = u32::from_be_bytes(r.segments[0].bytes[36..40].try_into().unwrap());
    assert_eq!(w9, (op::TRAP as u32) << 24);

    // String data starts at offset 40
    assert_eq!(&r.segments[0].bytes[40..53], b"Hello, World!");
    assert_eq!(r.segments[0].bytes[53], b'\n');
    assert_eq!(r.segments[0].bytes[54], 0);
}
...
fn assemble_case_insensitive_mnemonic() {
    ...
    assert_eq!(r1.segments[0].bytes, r2.segments[0].bytes);
}
...
\n\
    ...
    assert_eq!(r.segments[0].bytes.len(), 40);

    // SET $0,$1 -> ORI $0,$1,0
    let w_set0 = u32::from_be_bytes(r.segments[0].bytes[20..24].try_into().unwrap());
    assert_eq!(w_set0 >> 24, op::ORI as u32);

    // SET $1,$3 -> ORI $1,$3,0
    let w_set1 = u32::from_be_bytes(r.segments[0].bytes[24..28].try_into().unwrap());
...
fn loc_branch_within_relocated_code() {
    ...
    let r = assemble(src).unwrap();
    assert_eq!(r.entry_addr, 0x100);
    let bz_word = u32::from_be_bytes(r.segments[0].bytes[0..4].try_into().unwrap());
...
fn loc_multiple_forward_gap_zero_padded() {
    ...
    let r = assemble(src).unwrap();
    assert_eq!(r.entry_addr, 0x100);
    assert_eq!(r.segments[0].bytes.len(), 20); // 4 + 12 + 4
    assert_eq!(&r.segments[0].bytes[4..16], &[0u8; 12]);
    // SETL starts at bytes[16].
    let setl_word = u32::from_be_bytes(r.segments[0].bytes[16..20].try_into().unwrap());
...
// ─── Segment symbol tests ───

#[test]
fn segment_symbols_predefined() {
    // LOC Data_Segment sets entry_addr to 0x2000_0000_0000_0000.
    let src = "        LOC Data_Segment\n        TRAP 0,0,0";
    let r = assemble(src).unwrap();
    assert_eq!(r.entry_addr, 0x2000_0000_0000_0000);
    assert_eq!(r.segments.len(), 1);
    assert_eq!(r.segments[0].base, 0x2000_0000_0000_0000);
    assert_eq!(r.segments[0].bytes.len(), 4);
}

#[test]
fn two_segment_program() {
    // Code in Text segment then data in Data segment → two disjoint segments.
    let src = "        LOC #0\n        TRAP 0,0,0\n        LOC Data_Segment\n        OCTA 42";
    let r = assemble(src).unwrap();
    assert_eq!(r.segments.len(), 2);
    assert_eq!(r.segments[0].base, 0);
    assert_eq!(r.segments[0].bytes.len(), 4);
    assert_eq!(r.segments[1].base, 0x2000_0000_0000_0000);
    assert_eq!(r.segments[1].bytes.len(), 8);
}

#[test]
fn intra_segment_loc_pads() {
    // LOC within the same MMIX segment → zero-padding, single output segment.
    let src = "        LOC #0\n        TRAP 0,0,0\n        LOC #10\n        TRAP 0,0,0";
    let r = assemble(src).unwrap();
    assert_eq!(r.segments.len(), 1);
    // 4 bytes TRAP + 12 bytes zero pad + 4 bytes TRAP = 20 bytes
    assert_eq!(r.segments[0].bytes.len(), 20);
    assert_eq!(&r.segments[0].bytes[4..16], &[0u8; 12]);
}

#[test]
fn reserved_symbol_error() {
    // Redefining a canonical segment symbol via IS is a hard error.
    let err = assemble("Data_Segment IS 42").unwrap_err();
    assert!(err.message.contains("reserved"), "unexpected error: {}", err.message);
}

#[test]
fn segment_interleave_text_data_text() {
    // Typical MMIX layout: early text stubs, data segment, then main at #100.
    let src = "        LOC #0\n        TRAP 0,0,0\n        LOC Data_Segment\n        OCTA 42\n        LOC #100\n        TRAP 0,0,0";
    let r = assemble(src).unwrap();

    assert_eq!(r.segments.len(), 2);

    // Sorted-by-base output guarantees Text before Data.
    assert_eq!(r.segments[0].base, 0);
    assert_eq!(r.segments[0].bytes.len(), 0x104);
    assert_eq!(u32::from_be_bytes(r.segments[0].bytes[0..4].try_into().unwrap()), (op::TRAP as u32) << 24);
    assert!(r.segments[0].bytes[4..0x100].iter().all(|&b| b == 0));
    assert_eq!(u32::from_be_bytes(r.segments[0].bytes[0x100..0x104].try_into().unwrap()), (op::TRAP as u32) << 24);

    assert_eq!(r.segments[1].base, 0x2000_0000_0000_0000);
    assert_eq!(r.segments[1].bytes, 42u64.to_be_bytes().to_vec());
}
